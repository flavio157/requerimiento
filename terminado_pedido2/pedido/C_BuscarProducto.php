...
require_once("M_BuscarProductos.php");
    ...
    if($accion === "buscar"){
       ...
       // $bono = $_POST['bono'];
        $zona = $_POST['zona'];
        C_BuscarProducto::PoliticaPrecios($zona,$cantidad,$cod_producto);
...
class C_BuscarProducto
{
        ...
        $M_buscarproducto = new M_BuscarProductos();
            if(substr($nomproducto, 0, 2) == "CM"){
                ...
                foreach($c_cod as $producto){
                    $html.= '<div><a class="suggest-element" datagramo="'.$producto['GRAMOS'].'" data="'.$producto['DES_PRODUCTO'].'&'.$producto['PRECIO'].'"  
   ...
   /* static function vrificarprecio($zona,$cantidad,$dt){
    ...
    static function PoliticaPrecios($zona,$cantidad,$cod_producto){
            $M_politicaPrecio = new M_BuscarProductos();
