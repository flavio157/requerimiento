    ...
    class c_comprobante
    {
        ...
        static function c_guardarcompro($fechemision, $horaemision,$fechentrega,$codpersonal
           ...
           $nrocompro = strtoupper($nrocompro); 
            //print_r($tipocomprob."!=". $nrocompro[0]);       
            if($tipocomprob != $nrocompro[0]){print_r("Numero del comprobate debe iniciar con el tipo"); return;}
            $pattern = "/^[a-zA-Z\sñáéíóúÁÉÍÓÚ]+$/";
            ...
            if(strlen($observacio) > 500){print_r("Error sobrepaso el limite de caracteres"); return;}
            if(count($productos->tds)==0){print_r("Error ingrese Materiales"); return;}
