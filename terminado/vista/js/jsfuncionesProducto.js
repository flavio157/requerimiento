    ...
    function validacionFrm() {
        ...
        if($("#txtnumero").val().length == 0){
            ...
            console.log(verificar[verificar.length-1]);

            if( verificar[2] === undefined && verificar.length < 2 || verificar[verificar.length-1] === "") {
                mensajesError("se necesita un nombre y los dos apellidos","mensajesgenerales");
              $('#txtcliente').focus();

            }else if ($("#txtdireccion").val().length > 0){
                    verificar = $("#txtdireccion").val().split(" ");
                    if( verificar[1] === undefined && verificar.length < 2 || verificar[verificar.length-1] === "") {
