   ...
   class M_Pedidos 
   {
    ...
    $nuevaFecha = date("d-m-Y", strtotime($fcancelacion));
        ...
        try { 
            ...
            $query1=$this->bd->prepare("INSERT INTO T_CALL_CENTER(NUM_CONTRATO,NOMBRE,
            DIRECCION,OFICINA,FEC_OK,COD_PERSONAL,IDENTIFICACION,COD_REGISTRO,FEC_REGISTRO,EQUIPO,
            COD_DISTRITO,CUOTAS,COMENTARIO,DESPRODUCTOS,
            MON_INICIAL,FEC_GENERADO,COD_VENDEDOR,COD_ZONA,REFERENCIA,COD_CLIENTE,TELEFONO2) values 
            
            (:num_contrato,:nombre,:direccion,:oficina,:feh_ok,:cod_personal,
            :identificacion,:cod_registro,:fec_registro,:equipo,:cod_distrito,
            :cuotas,:comentario,:desproductos,:mon_inicial,:fec_generado,
            :cod_vendedor,:cod_zona,:contado)");
            ...
            $query2 = $this->bd->prepare("INSERT INTO T_CALL_CENTER_ITEM(NUM_CONTRATO,
                                        COD_PRODUCTO,NUM_LOTE,PRECIO,OFICINA) 
                                        VALUES(:contrato,:cod_producto,:lote,:precio,:oficina)");
            foreach ($dataproductos->arrayproductos as $dato){
                if(isset($dato->cod_producto)){
                    ...
                    $query2->bindParam("contrato",$num_contrato,PDO::PARAM_STR);
                    $query2->bindParam("cod_producto",$dato->cod_producto,PDO::PARAM_STR);
                    $query2->bindParam("lote",$dato->cantidad,PDO::PARAM_STR);
                    $query2->bindParam("precio",$dato->precio,PDO::PARAM_STR);
                    $query2->bindParam("oficina", $dato->oficina,PDO::PARAM_INT);
